...
private val englishText = mapOf(
    ...
    "统计信息" to "Statistics",
    "图标总数" to "Total icons",
    "映射总数" to "Total mappings",
    "实际图标数量" to "Actual icons",
    ...
    "前景图层" to "Foreground layer",
    "完整原始图层" to "Full original layers",
    ...
    "这些模式只决定 Source Image 如何提取；之后都会统一经过可选 Background、模板 Mask 和 Overlay。" to "These modes only change how the source image is extracted; they will all go through the optional Background, template Mask, and Overlay.",
    ...
    "应用图标包背景" to "Apply icon-pack background",
    "清空背景" to "Clear background",
    "清空" to "Clear",
    ...
    "正在扫描模板资源" to "Scanning template resources",
    "正在写入图标与映射" to "Writing icons and mappings",
    "正在处理模板资源" to "Processing template resources",
    ...
    "隐藏详情" to "Hide details",
    "显示详情" to "Show details",
    ...
    "已适配全部已安装应用" to "All installed apps are covered",
    "搜索" to "Search",
    ...
    "下载" to "Download",
    "移除" to "Remove",
    "例如：com.example.app" to "Example: com.example.app",
